...
function applyLanguage(lang) {
  ...
  $("h1").text(t.title);
  $("#new-task-input").attr("placeholder", t.placeholder);
  $("#new-task-submit").val(t.addButton);
  $(".task-list h2").text(t.taskHeader);
}

// Seçili dili localStorage'tan al, yoksa 'en' varsayılan
const savedLang = localStorage.getItem("lang") || "en";
$("#language-select").val(savedLang);
applyLanguage(savedLang);

// Seçim değişince dil değiştir
